...
function CalendarCarousel<T>({
  ...
  const renderCalendars = (): ReactElement => {
    const renderCalendar = (displayDate: Date): ReactElement => {
      ...
      const renderDates = (dateItem: Date): ReactElement => {
        ...
        if (itemMonth !== month) {
          ...
          return (
            <TouchableOpacity
              onPress={(): void => {
                selectDate && selectDate(setItemDay);
                setEventDay(itemDay);
              }}
            >
          ...
          return (
            <TouchableOpacity
              onPress={(): void => selectDate && selectDate(setItemDay)}
            >
              <View style={styles.defaultView} key={itemDay}>
      ...
      const markedDates = markedDayEvents.map(
        (markeddates: { selectedEventDate: { getDate: () => any } }) =>
          markeddates.selectedEventDate.getDate()
      );
      const markedMonths = markedDayEvents.map(
        (markedmonths: { selectedEventDate: { getMonth: () => number } }) =>
          markedmonths.selectedEventDate.getMonth() - 1
      );
      const markedYears = markedDayEvents.map(
        (markedyears: { selectedEventDate: { getFullYear: () => any } }) =>
